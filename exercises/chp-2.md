...
4. 데이터 사전에 저장된 데이터는 한 차원 높은 데이터라는 의미에서 메타 데이터 (meta data) 라고 한다. (__O__)
...
2. 데이터베이스 관리자인 DBA는 (Database Adminstrator)의 약어이다.
...
- 제어 기능: 데이터의 무결성, 보안, 병행 제어 등

**18 데이터 모델에 따른 DBMS 의 역사에 대해 간략히 설명하시오.**
- 계층형 모델: 트리 구조로 데이터 저장
- 망형 모델: 그래프 구조로 복잡한 관계 표현
- 관계형 모델: 표 형태로 데이터를 저장하고 관리
- 객체-관계형 모델: 객체지향 개념을 도입하여 관계형 모델을 확장

**19 데이터 사전(data dictionary)과 데이터 디렉토리 (data directory)의 차이점을 설명하시오.**
- 데이터 사전: 데이터베이스에 저장된 데이터에 대한 정보를 저장
- 데이터 디렉토리: 데이터 파일의 물리적 위치와 관련된 정보를 저장

**20 다양한 수준의 추상화와 테이터 독립성율 제공하기 위하여 대이터베이스 관리 시스템 (DBMS)에서 지원하는 3 단계 스키마 구조에 대해셔 간략히 설명하시오.**
- 외부 스키마: 사용자 관점에서 본 데이터 구조
- 개념 스키마: 조직 전체의 데이터 구조
- 내부 스키마: 데이터의 물리적 저장 구조
