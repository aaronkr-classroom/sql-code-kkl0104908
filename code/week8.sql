...
SELECT 신청날짜,
	...
	TO_CHAR(신청날짜, 'YYYY년''MM월''DD일') AS 형식2
FROM 수강;

-- 저장 프로시저
CREATE OR REPLACE PROCEDURE InsertOfUpdateCourse(
	...
	ELSE
		-- 과목이 존재하면 기존 과목 업데이트
		UPDATE 과목
		SET 이름 = CourseName, 강의실 = CourseRoom, 개설학과 = CourseDept, 시수 = CorseCredit
		WHERE 과목번호 = CourseNo;
	END IF;
END;
$$;
	...
	-- 새 과목 추가하기
	CALL InsertOfUpdateCourse('c006', '연극학개론', '310', '교양학부', 2);
	SELECT * FROM 과목;
	
	-- 과목 업데이트 하기
	CALL InsertOfUpdateCourse('c006', '연극학개론', '410', '교양학부', 1);
	SELECT * FROM 과목;
...
-- CALL SelectAverAgeBestScore(95, @Count);
-- SELECT @Count;
...
CREATE TABLE 남녀학생총수
	...
	PRIMARY KEY (성별));
INSERT INT 남녀학생총수 SELECT '남', COUNT(*) FROM 학생 WHERE 성별 = '남';
INSERT INT 남녀학생총수 SELECT '여', COUNT(*) FROM 학생 WHERE 성별 = '여';
SELECT * FROM 남녀학생총수;
...
-- 사용자 정의한 함수
CREATE OF REPLACE FUNCTION Fn_GRade(grade CHAR)
RETURN TEXT AS $$
BEGIN
	CASE grade
